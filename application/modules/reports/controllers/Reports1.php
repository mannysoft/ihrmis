...
class Reports extends MX_Controller
{
	...
	function statement_leave($state_no = '', $employee_id = '')
	{
		...
		// set the sourcefile
		$pdf->setSourceFile('dtr/template/statement_leave4.pdf');
		...
		while(date('Y') >= $year)
		{
			...
			if($x == 0)
			{
				...
				{
					$pdf->Write(0, $month.'/'.$day.'/'.date("y", mktime(0, 0, 0, $month, $day, $year)).'  to  12/31/'.date("y", mktime(0, 0, 0, $month, $day, $year)));
				}
			}
			...
			else
			{
				...
				if($year_count == $year_diff)
				{
					//get the last month that earned leave credit
					$last_month_last_day = date('Y-m-d',strtotime('-1 second',strtotime(date('m').'/01/'.date('Y').' 00:00:00')));
		
					//list($year3, $month3, $day3) = split('[-.-]', $last_month_last_day);
					list($year3, $month3, $day3) = explode('-', $last_month_last_day);
					
					$last_month_last_day = date("m/d/y", mktime(0, 0, 0, $month3, $day3, $year3));
					//$pdf->Write(0, $last_month_last_day);
					$pdf->Write(0, '01/01/'.date("y", mktime(0, 0, 0, $month, $day, $year)).'  to  '.$last_month_last_day);
				}
				
				else
				{
				
					$pdf->Write(0, '01/01/'.date("y", mktime(0, 0, 0, $month, $day, $year)).'  to  12/31/'.date("y", mktime(0, 0, 0, $month, $day, $year)));
				}
			}
