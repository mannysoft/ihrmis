...
<table id="dtr.table" width="100%" border="0" class="type-one">
  ...
  <?php 
	...
	foreach($rows as $row)
	{
		...
		//list($log_year, $log_month, $log_day) = split('[-.-]', $row['log_date']);deprecated
...
<?php if ( $this->session->userdata('user_type') == 6 or $this->session->userdata('user_type') == 8 or $this->session->userdata('user_type') == 5):?>
