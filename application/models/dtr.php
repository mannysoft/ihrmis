 ...
 * @license		http://charliesoft.net/atlms/user_guide/license.html
 ...
 * @category	Models
 * @author		Manolito Isles
 * @link		http://charliesoft.net/atlms/user_guide/models/dtr.html
 */
class Dtr extends CI_Model {
	
	// --------------------------------------------------------------------
	
	var $fields 	= array();	// Fields in the table dtr that needs to be selected
	var $is_pm_in12 = FALSE;
	...
	function get_dtr_range($date1, $date2, $web_service = FALSE)
	{
		...
		// To do: use this for web services
		/*
		if(date('a') == 'am')
			{
				$this->db->select('employee_id, log_date, am_login, am_logout, office_id, shift_id');
			}
			if(date('a') == 'pm')
			{
				$this->db->select('employee_id, log_date, am_logout, pm_login, pm_logout, office_id, shift_id');
			}
