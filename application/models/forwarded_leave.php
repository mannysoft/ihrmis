 ...
 */
class Forwarded_leave extends CI_Model{
								 ...
								 $notes = '', $date_cutoff)
	{
		...
		$this->db->delete('forwarded_leave', array('employee_id' => $employee_id)); 
		...
		$this->db->insert('forwarded_leave', $data);
	...
	function update_forward_leave($employee_id, $field, $new_value)
	{
		...
		else
		{
			...
			$this->db->where('employee_id', $employee_id);
			$this->db->update('forwarded_leave', $data);
...
/* End of file forwarded_leave.php */
/* Location: ./application/models/forwarded_leave.php */