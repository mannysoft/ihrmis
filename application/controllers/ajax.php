...
class Ajax extends MX_Controller {
						...
						$allow_sat_sun = 0, $hospital_leave_days = '')
	{
		...
		if ($process || $process == 2)
		{
			...
			{	
				...
				//echo "onClick=".'"'."openBrWindow("."''".','."''".','scrollbars=yes,width=900,height=600')"";
				//$link_print_preview = base_url().'reports/leave_apps/'.$leave_apps_id;
				//echo 'onClick="openBrWindow('."'".$link_print_preview."'".',';
				//echo "'','"."scrollbars=yes,width=900,height=600'".')"';
				
				//$onclick = 'onClick="openBrWindow('."'".$link_print_preview."'".',';
				//$onclick .= "'','"."scrollbars=yes,width=900,height=600'".')"';
				
				//$onclick = '';
				
				//$link_print_preview .= ' reports/leave_apps/';
	...
	function file_cto( $process = '', $id = '' )
	{
		...
		if ($this->input->post('id') != '')
		{
			...
			//$leave_apps_id = $this->cto->multiple;
